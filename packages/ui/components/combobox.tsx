    ...
    return (
      <div className="relative flex-1">
        <ComboboxPrimitive.Input
          className={cn(
            "w-full min-w-12 text-base outline-none sm:text-sm [[data-slot=combobox-chip]+&]:ps-0.5",
            sizeValue === "sm" ? "ps-1.5" : "ps-2",
            endAddon && (sizeValue === "sm" ? "pe-6.5" : "pe-7"),
            className,
          )}
        ...
        {endAddon && (
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-y-0 end-px z-10 flex items-center pe-[calc(--spacing(3)-1px)] opacity-80 [&_svg]:-mx-0.5 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4"
    ...
    <div className="text-foreground relative w-full not-has-[>*.w-full]:w-fit has-disabled:opacity-64">
      ...
      <ComboboxPrimitive.Input
        className={cn(
          ...
          endAddon &&
            "*:data-[slot=combobox-input]:pe-[calc(--spacing(8.5)-1px)] data-[size=sm]:*:data-[slot=combobox-input]:pe-[calc(--spacing(7.5)-1px)] sm:*:data-[slot=combobox-input]:pe-[calc(--spacing(8)-1px)] sm:data-[size=sm]:*:data-[slot=combobox-input]:pe-[calc(--spacing(7)-1px)]",
          !endAddon && sizeValue === "sm"
      ...
      {endAddon && (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 end-px z-10 flex items-center pe-[calc(--spacing(3)-1px)] opacity-80 has-[+[data-size=sm]]:pe-[calc(--spacing(2.5)-1px)] [&_svg]:-mx-0.5 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4"
          data-slot="combobox-end-addon"
        >
          {endAddon}
        </div>
      ...
      >
        ...
        >
          <ComboboxPrimitive.Popup
            className="text-foreground flex max-h-[min(var(--available-height),23rem)] max-w-(--available-width) min-w-(--anchor-width) flex-col"
...
function ComboboxItem({ className, children, ...props }: ComboboxPrimitive.Item.Props) {
  return (
    <ComboboxPrimitive.Item
      className={cn(
        "data-highlighted:bg-accent data-highlighted:text-accent-foreground flex min-h-8 cursor-default items-center gap-2 rounded-sm px-2 py-1 text-base outline-none in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4",
        className,
      )}
...
function ComboboxItemIndicator({
  className,
  ...props
}: Omit<React.ComponentProps<typeof ComboboxPrimitive.ItemIndicator>, "children">) {
  return (
